...
class Autobus(Agent):
    ...
    def move(self):
        ...
        lista = self.model.grid.get_neighbors(self.posicion,True)
        semaforos = [obj for obj in lista if isinstance(obj, Semaforo)]
        if semaforos != []:
            if semaforos[0].Color == "green":
                if self.velocidad_actual < VEL_MAX:
                    self.velocidad_actual = self.velocidad_actual + ACELERACION * 10
                
                if self.velocidad_actual >= VEL_MAX:
                    self.velocidad_actual = VEL_MAX
                distancia = self.velocidad_actual * 10 + 0.5 * ACELERACION * 100
...
class Peaton(Agent):
    def __init__(self, unique_id, model, x, y, origen, destino):
        super().__init__(unique_id, model)
        self.posicion = (x, y)
        ...
        if( (self.hora_actual > baja1 and self.hora_actual < baja2) or (self.hora_actual > baja3 and self.hora_actual < baja4)):
            ...
            for paradero in self.paraderos:
                if self.schedule.steps % 18 == 0:
                    ...
                    print("Se agrego el peaton " , peaton.unique_id , " En el paradero " , paradero.unique_id)           
